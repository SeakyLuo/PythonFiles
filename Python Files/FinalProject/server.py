...
class server:
    def __init__(self, name, config: dict):
        ...
        self.sendSocket = {}
        self.ballot_pool = []
        self.replySocket = { name: socket(AF_INET, SOCK_STREAM) for name in names if name != self.name }
        # if log:
        ...
        while True:
            ...
            if message == self.name:
                ...
                threading.Thread(target = self.server, args = (eval(message), )).start()
    ...
    def prepare(self):
        ...
        message = Message(PREPARE, self.name, ballot)
        self.broadcast(message)
