...
integer=lambda x:[x,int(x)][int(x)==x]
...
class stats:
    ...
    def getInfo(self):
        ...
        mode=[str(list(self.mode))[1:-1],self.mode][self.mode==None]
        outliers=[str(self.outliers)[1:-1],self.outliers][self.outliers==None]
        self.info='''Count: {}
Min: {}
Max: {}
Mean: {}
...
Standard Deviation: {}
Average Deviation: {}
Outliers: {}'''.format(self.num,self.min,self.max,self.mean,self.median,mode,\
                        self.var,self.sd,self.ad,outliers)
            ...
            while nmin<nmax or pmin<pmax:
                for k in self.count:
                    ...
                    if nmin<=k<nupper:
                        ...
                        key='[{},{})'.format(pmin,pupper)
                    if key:
                        self.groups[key]=self.groups.get(key,0)+self.count[k]
                nmin//=interval
                pmin*=interval
            self.groups={ k:self.groups[k] for k in \
                          sorted(self.groups,key=lambda x:eval(ez.find(x).between('[',',')))}
        return self.groups
    ...
    def plot(self,interval=0,font=("Helvetica",16,'normal'),color="yellow",percent=False):
        ...
        while True:
            ...
            ct.goto(lower, 0)
            ct.down()
            value=self.groups.get('[{},{})'.format(lower,lower+interval),0)
            if value:
                ...
                ct.write(value,align='center',font=(font[0],[font[1],int(font[1]*0.75)][len(value)>2*interval],font[2]))
