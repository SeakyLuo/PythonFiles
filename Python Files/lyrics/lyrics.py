    ...
    def save(self,saveAs='lrc',ALL=True):
        ...
        if self.lyrics:
            ...
            if len(self.lyrics_list)==1 or not ALL:
                filename=self.title+' '+self.artist+'.'+saveAs
        ...
        if os.path.exists(self.path):
            ...
            for fmt in music_fmt:
                ...
                if score:
                    ...
                    text=[self.artist+'的',''][self.artist=='']+'《'+self.title+'》'
                    print('这个路径下只能找到'+text+'哦，自动调整成这首歌啦')
                else:
                    text=[self.artist+'的',''][self.artist=='']+'《'+self.title+'》'
                    raise Exception('QAQ这个路径下找不到'+text+'诶...')

        if not ez.startwith(path,defaultPath) and os.path.exists(defaultPath+self.path):
            have(path).start(defaultPath)
            self.path=defaultPath+self.path
        if not ez.endwith(self.path,'\\'):
            self.path+='\\'
        if self.title=='':
        ...
        try:
            ...
            if ow:
                if src:
                    ...
                    print('总共找到{}条歌词，已覆盖写入第1条√'.format(len(self.lyrics_list)))
            else:
                print('总共找到{}条歌词，已成功写入第1条~~~'.format(len(self.lyrics_list)))
        except KeyboardInterrupt:
            return
        except:
            self.fail_list.append((title,artist))
