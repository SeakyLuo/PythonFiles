...
resumeButton = lambda button: button.config(text = '', relief = RAISED)
pressButton = lambda button, player: button.config(text = player, relief = FLAT)
...
class FIR:
    def __init__(self, master, player = '0', opp = '*', mode = PVE):
        ...
        self.opp = opp
        self.empty = '_'
        self.endmsg = '游戏已经结束啦，按Restart可以重新开始游戏哦'
        self.mode = mode
        for i in range(16):
        ...
        self.undo = Button(self.master, text = 'Undo', command = self.undo)
        self.undo.grid(row = 1, column = 17)
        ...
        self.setup()
        print(f'游戏开始啦~你是{self.player}，电脑是{self.opp}')

    def setup(self):
        self.gameover = False
        self.playerLabel = Label(self.master, text = '玩家1' if self.mode == PVE else '你')
        ...
        self.oppLabel = Label(self.master, text = '玩家2' if self.mode == PVE else '电脑')
        ...
        self.buttons = {(i, j): self.empty for i in range(1, 16) for j in range(1, 16)}
        self.grid = self.buttons.copy()
        for i in range(1, 16):
            for j in range(1, 16):
        ...
        if choice([0, 1]):
            if self.mode == PVE:
                ...
                print(f'这次是玩家{self.opp}先哦')
        else:
            print('这次是'+(('玩家'+self.player) if self.mode == PVP else '你')+'先下哦')
    ...
    def restart(self):
        ...
        for loc in self.moves:
            resumeButton(self.buttons[loc])
        self.setup()

    def switchMode(self):
        self.mode = PVE if self.mode == PVP else PVP
        self.modeButton['text'] = self.mode
        self.playerLabel['text'] = '玩家1' if self.mode == PVP else '你'
        self.oppLabel['text'] = '玩家2' if self.mode == PVP else '电脑'
    ...
    def put(self, button):
        ...
        r = info['row']
        c = info['column']
        pressButton(button, self.player)
        self.moves.append((r, c))
        self.grid[(r, c)] = self.player
        if self.mode == PVE:
            print(f'你把棋放在了({HEX(r)}, {HEX(c)})')
            self.respond()
        else:
            print(f'玩家{self.player}把棋放在了({HEX(r)}, {HEX(c)})')
            if self.check(self.player)[0] == 5:
                self.end(f'游戏结束啦，{self.player}获得胜利~')
            else:
    ...
    def respond(self):
        ...
        pressButton(self.buttons[(r, c)], self.opp)
        self.moves.append((r, c))
        print(f'电脑把棋放在了({HEX(r)}, {HEX(c)})')
